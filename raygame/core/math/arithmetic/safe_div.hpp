...
#include "raygame/core/math/math.hpp"
...
constexpr Out_T safe_div(const std::integral auto aval, const std::integral auto bval) {
    if (std::cmp_equal(bval, 0)) {
        throw core::exception::Condition("Cannot divide by zero");
    }
    if (std::cmp_equal(bval, -1) && std::cmp_equal(aval, std::numeric_limits<Out_T>::lowest())) {
    ...
    if constexpr (std::is_signed<decltype(aval)>() == std::is_signed<decltype(bval)>()) {
