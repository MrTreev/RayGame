...
void EvilWizard::set_position(raylib::Vector2 position) {
    _position = position;
};

void EvilWizard::move(raylib::Vector2 position) {
    _position += position;
};

raylib::Vector2 EvilWizard::get_position() {
    return _position;
};

void EvilWizard::action(EvilWizardSprite action) {
    _frameno  = 0;
    _sprite   = action;
    _swing_up = !_swing_up;
};

void EvilWizard::draw_attack_down() {
    _attack_down.draw(_position, _frameno);
};

void EvilWizard::draw_attack_up() {
    _attack_up.draw(_position, _frameno);
};

void EvilWizard::draw_death() {
    _death.draw(_position, _frameno);
};

void EvilWizard::draw_fall() {
    _fall.draw(_position, _frameno);
};

void EvilWizard::draw_hit() {
    _hit.draw(_position, _frameno);
};

void EvilWizard::draw_idle() {
    _idle.draw(_position, _frameno);
};

void EvilWizard::draw_jump() {
    _jump.draw(_position, _frameno);
};

void EvilWizard::draw_run() {
    _run.draw(_position, _frameno);
};

void EvilWizard::draw() {
    _frameno++;
    switch (_sprite) {
