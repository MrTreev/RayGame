...
#define MINTEST(x) std::numeric_limits<x>::lowest() - 1
#define MAXTEST(x) std::numeric_limits<x>::max() + 1

TTS_CASE("numeric_cast throws") {
    ...
    TTS_THROW(numeric_cast<int64_t>(MINTEST(int64_t)), Condition);
    TTS_THROW(numeric_cast<int64_t>(MAXTEST(int64_t)), Condition);
    ...
    TTS_THROW(numeric_cast<uint64_t>(MINTEST(uint64_t)), Condition);
};
