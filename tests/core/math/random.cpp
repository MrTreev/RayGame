...
RG_TYPED_TEST_SUITE(Rand, test::types::Integral);
...
RG_TYPED_TEST(Rand, Rand) {
    ...
    RG_CHECK_FALSE(all_same(test_arr));
    RG_CHECK_TRUE(std::all_of(test_arr.begin(), test_arr.end(), [](auto val) {
        return val >= min_rand;
    }));
    RG_CHECK_TRUE(std::all_of(test_arr.begin(), test_arr.end(), [](auto val) {
        return val <= max_rand;
...
RG_TYPED_TEST(Rand, RandN) {
    using T = decltype(this->val());
    const T              max_rand{1};
    const T              min_rand{0};
    const std::vector<T> test_vec = rand_n<T>(TEST_LEN);
    const std::vector<T> rand_01  = rand_n<T>(TEST_LEN, min_rand, max_rand);
    RG_CHECK_EQ(test_vec.size(), TEST_LEN);
    RG_CHECK_FALSE(all_same(test_vec));
    RG_CHECK_EQ(rand_01.size(), TEST_LEN);
    RG_CHECK_FALSE(all_same(rand_01));
    for (const auto& val: rand_01) {
        RG_CHECK_GE(val, min_rand);
        RG_CHECK_LE(val, max_rand);
    }
}
